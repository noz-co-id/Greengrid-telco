...
write_api = None
mqtt_client = None
telco_manager = None  # Telco Core Network Manager
...
def init_telco_simulator():
    """Initialize Telco Core Network Simulator"""
    global telco_manager
    try:
        telco_manager = TelcoCoreNetworkManager()
        telco_manager.start_simulation()
        logger.info("Telco Core Network Simulator initialized")
        
        # Start telco traffic generator thread
        telco_thread = threading.Thread(target=telco_traffic_generator, daemon=True)
        telco_thread.start()
        logger.info("Telco traffic generator started")
    except Exception as e:
        logger.error(f"Failed to initialize Telco Simulator: {e}")

def telco_traffic_generator():
    """Generate telco protocol traffic and publish to MQTT"""
    while True:
        try:
            if telco_manager and mqtt_client:
                # Generate network traffic
                traffic = telco_manager.generate_network_traffic()
                
                # Publish each message
                for message in traffic:
                    topic = f"greengrid/telco/{message['protocol']}"
                    payload = json.dumps(message)
                    mqtt_client.publish(topic, payload, qos=0)
                    
                    # Also store in InfluxDB
                    if write_api:
                        point = Point("telco_signaling") \
                            .tag("protocol", message['protocol']) \
                            .tag("procedure", message['procedure']) \
                            .field("imsi", message.get('imsi', 'unknown')) \
            ...
            logger.error(f"Error in telco traffic generator: {e}")
            time.sleep(5)
...
def get_dashboard_summary():
    ...
    try:
        ...
        telco_stats = {}
        if telco_manager:
            telco_stats = telco_manager.get_all_statistics()
        ...
        summary = {
            ...
            'telco_networks': telco_stats.get('networks', {}),
            'total_active_sessions': telco_stats.get('total_active_sessions', 0)
        }
...
def main():
    ...
    init_influxdb()
    init_mqtt()
    init_telco_simulator()  # Initialize telco core network simulator
